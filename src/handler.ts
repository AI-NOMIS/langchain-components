...
export class CustomChainHandler extends BaseCallbackHandler {
  ...
  handleChainEnd(outputs: ChainValues): void | Promise<void> {
    if (this.returnSourceDocuments) {
      this.socketIO
        .to(this.socketIOClientId)
        .emit("sourceDocuments", outputs?.sourceDocuments);
    }
  }
}

export const additionalCallbacks = async (
  nodeData: INodeData,
    ...
    for (const provider in analytic) {
      ...
      if (providerStatus) {
        ...
        if (provider === "langSmith") {
          ...
          const langSmithApiKey = getCredentialParam(
            "langSmithApiKey",
            credentialData,
            nodeData
          );
          const langSmithEndpoint = getCredentialParam(
            "langSmithEndpoint",
            credentialData,
            nodeData
          );
          ...
          const langFusePublicKey = getCredentialParam(
            "langFusePublicKey",
            credentialData,
            nodeData
          );
          const langFuseEndpoint = getCredentialParam(
            "langFuseEndpoint",
            credentialData,
            nodeData
          );
          ...
          if (flushInterval)
            langFuseOptions.flushInterval = parseInt(flushInterval, 10);
          if (requestTimeout)
            langFuseOptions.requestTimeout = parseInt(requestTimeout, 10);
          ...
          const llmonitorAppId = getCredentialParam(
            "llmonitorAppId",
            credentialData,
            nodeData
          );
          const llmonitorEndpoint = getCredentialParam(
            "llmonitorEndpoint",
            credentialData,
            nodeData
          );
