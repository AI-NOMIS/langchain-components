...
class Qdrant_Existing_VectorStores implements INode {
  ...
  async init(
    ...
    let qdrantCollectionConfiguration =
      nodeData.inputs?.qdrantCollectionConfiguration;
    ...
    const qdrantApiKey = getCredentialParam(
      "qdrantApiKey",
      credentialData,
      nodeData
    );
    ...
    if (queryFilter) {
      retrieverConfig.filter =
        typeof queryFilter === "object" ? queryFilter : JSON.parse(queryFilter);
    }
