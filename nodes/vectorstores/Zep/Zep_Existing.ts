  ...
  async init(
    ...
    if (zepMetadataFilter) {
      const metadatafilter =
        typeof zepMetadataFilter === "object"
          ? zepMetadataFilter
          : JSON.parse(zepMetadataFilter);
      zepConfig.filter = metadatafilter;
    }

    const vectorStore = await ZepExistingVS.fromExistingIndex(
      embeddings,
      zepConfig
    );
...
class ZepExistingVS extends ZepVectorStore {
  ...
  static async fromExistingIndex(
    ...
    dbConfig: IZepConfig & Partial<ZepFilter>
