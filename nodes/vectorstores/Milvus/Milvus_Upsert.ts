...
class Milvus_Upsert_VectorStores implements INode {
  ...
  async init(
    ...
    const milvusUser = getCredentialParam(
      "milvusUser",
      credentialData,
      nodeData
    );
    const milvusPassword = getCredentialParam(
      "milvusPassword",
      credentialData,
      nodeData
    );
    ...
    const vectorStore = await MilvusUpsert.fromDocuments(
      finalDocs,
      embeddings,
      milVusArgs
    );
    ...
    vectorStore.similaritySearchVectorWithScore = async (
      ...
      const outputFields = vectorStore.fields.filter(
        (field) => field !== vectorStore.vectorField
      );
      ...
      searchResp.results.forEach((result) => {
        ...
        Object.keys(result).forEach((key) => {
          if (key === vectorStore.textField) {
            ...
            vectorStore.fields.includes(key) ||
            key === vectorStore.primaryField
          ) {
