...
class Airtable_Agents implements INode {
  ...
  async run(
    ...
    let base64String = Buffer.from(JSON.stringify(airtableData)).toString(
      "base64"
    );
...
json.dumps(my_dict)`;
    ...
    if (pythonCode) {
      try {
        ...
        throw new Error(
          `Sorry, I'm unable to find answer for question: "${input}" using follwoing code: "${pythonCode}"`
        );
      }
    }
