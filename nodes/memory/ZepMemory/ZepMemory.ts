...
class ZepMemory_Memory implements INode {
  ...
  constructor() {
    ...
    this.credential = {
      ...
      optional: true,
      description:
        "Configure JWT authentication on your Zep instance (Optional)",
  ...
  async init(
    ...
    zep.loadMemoryVariables = async (values) => {
      ...
      if (
        ...
        zep.returnMessages &&
        data[zep.memoryKey] &&
        data[zep.memoryKey].length
      ) {
        const zepClient = await zep.zepClientPromise;
        const memory = await zepClient.memory.getMemory(
          zep.sessionId,
          parseInt(k, 10) ?? 10
        );
        if (memory?.summary) {
          let summary = autoSummaryTemplate.replace(
            /{summary}/g,
  ...
  async clearSessionMemory(
    ...
    options.logger.info(
      `Clearing Zep memory session ${sessionId ? sessionId : chatId}`
    );
    await zep.clear();
    options.logger.info(
      `Successfully cleared Zep memory session ${
