...
class DynamoDb_Memory implements INode {
  ...
  async clearSessionMemory(
    ...
    options.logger.info(
      `Clearing DynamoDb memory session ${sessionId ? sessionId : chatId}`
    );
    await dynamodbMemory.clear();
    options.logger.info(
      `Successfully cleared DynamoDb memory session ${
...
const initalizeDynamoDB = async (
  ...
  const secretAccessKey = getCredentialParam(
    "secretAccessKey",
    credentialData,
    nodeData
  );
