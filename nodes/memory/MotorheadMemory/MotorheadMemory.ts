...
class MotorMemory_Memory implements INode {
  ...
  constructor() {
    ...
    this.credential = {
      ...
      optional: true,
      description:
        "Only needed when using hosted solution - https://getmetal.io",
  ...
  async clearSessionMemory(
    ...
    options.logger.info(
      `Clearing Motorhead memory session ${sessionId ? sessionId : chatId}`
    );
    await motorhead.clear();
    options.logger.info(
      `Successfully cleared Motorhead memory session ${
...
const initalizeMotorhead = async (
  nodeData: INodeData,
  options: ICommonObject
...
class MotorheadMemoryExtended extends MotorheadMemory {
  ...
  constructor(
    fields: MotorheadMemoryInput & Partial<MotorheadMemoryExtendedInput>
  ) {
    super(fields);
