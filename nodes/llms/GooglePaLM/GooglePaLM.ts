...
class GooglePaLM_LLMs implements INode {
  ...
  async init(
    ...
    const googleMakerSuiteKey = getCredentialParam(
      "googleMakerSuiteKey",
      credentialData,
      nodeData
    );
    ...
    if (stopSequencesObj) {
      try {
        parsedStopSequences =
          typeof stopSequencesObj === "object"
            ? stopSequencesObj
            : JSON.parse(stopSequencesObj);
        obj.stopSequences = parsedStopSequences.list || [];
      } catch (exception) {
        throw new Error(
          "Invalid JSON in the GooglePaLM's stopSequences: " + exception
        );
      }
    }
